...
import { PosterArtworkStage, type PosterCardPresentation } from "@/components/shared/PosterArtworkStage";
...
/**
 * Unified home poster card — used by both Featured Posters and New Arrivals.
 * Reads `posterCardPresentation` from StorefrontContext so it reacts to the
 * runtime DB config and renders through PosterArtworkStage (3 modes + 600 ms
 * hover-mockup crossfade), identical to the shop grid.
 */
function HomePosterCard({
  ...
  // Hover image resolution — identical strategy to PosterCard / shop grid:
  // 1. dedicated hoverDisplayImageUrl, 2. primaryDisplayImageUrl if differs from base, 3. null
  ...
  // Reads from StorefrontContext — re-renders automatically once the async
  // DB config resolves, so there is no stale-static-config issue.
  const presentation: PosterCardPresentation =
    (store.posterCardPresentation as PosterCardPresentation | null | undefined) ?? "current";
  ...
  return (
    ...
    >
      ...
      <div className="mt-1.5 min-w-0">
        ...
        <p
          className="text-xs font-medium text-foreground/70 mt-0.5"
...
function HomePosterCardSkeleton() {
  return (
    <div>
      <div className="aspect-[3/4] bg-muted animate-pulse" />
      <div className="mt-1.5 h-3 bg-muted animate-pulse w-3/4" />
