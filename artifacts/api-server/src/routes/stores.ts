...
const createStoreSchema = z.object({
  ...
  shopConfig: z.object({
    cardStyle: z.enum(["simple", "visual"]).nullable().optional(),
  }).nullable().optional(),
...
router.post("/admin/stores", requireAdmin, async (req, res) => {
  ...
  const [created] = await db
    ...
    .values({
      ...
      shopConfig: parsed.data.shopConfig ?? null,
      createdAt: now,
      updatedAt: now,
    })
    .returning();
...
router.put("/admin/stores/:storeKey", requireAdmin, async (req, res) => {
  ...
  if ("shopConfig" in parsed.data) updates.shopConfig = parsed.data.shopConfig ?? null;
...
router.get("/stores/:storeKey/config", async (req, res) => {
  ...
  return res.json({
    ...
    shopConfig: (store.shopConfig as object) ?? null,
  });
});
